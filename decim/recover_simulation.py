...
def par_execute(ii, chunks):
    ...
    with Pool(16) as p:
        ...
        for chunk in chunks:
            ...
            print(chunk)
            values = p.starmap(execute_dblfit, chunk)
            results.extend(values)
            df = pd.DataFrame(values)
            df.to_hdf(
                '/work/faty014/kobe_param_rec_dblfit_%i.hdf'%(ii), key='chunk%i'%cnt)
            cnt += 1

def execute_dblfit(H, V, gv, i, var, model, fixed_variable, parameters, isi, trials):
    ...
    points = pt.fast_sim(total_trials, tH=H, isi=isi)
